...
**Before creating a new numbered spec:** run `just specs-check-neighbors --changed NNN-your-new-spec.md` (or the equivalent for the spec you're drafting, once the file exists) to see which existing specs it reads most similarly to. If a returned neighbor genuinely covers the same feature, update that spec instead of adding a new one. When you do create a new spec that extends or narrows an existing one, name it explicitly in the **Depends on** field or in prose (e.g. "spec 014", "specs 017-020") — the neighbor check reads exactly that convention to tell a declared relationship from an undeclared one. This check is advisory (a ranked nearest-neighbor report, not a pass/fail gate) — see `src/codesight/spec_duplication.py` for why a hard threshold isn't used.
