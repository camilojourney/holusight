# Graph Report - holusight  (2026-08-15)

## Corpus Check
- 129 files · ~93,405 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1800 nodes · 2369 edges · 136 communities (127 shown, 9 thin omitted)
- Extraction: 94% EXTRACTED · 6% INFERRED · 0% AMBIGUOUS · INFERRED: 135 edges (avg confidence: 0.51)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `1e99ecac`
...
1. `CodeSight` - 59 edges
2. `ChunkStore` - 47 edges
3. `ServerConfig` - 46 edges
4. `SearchResult` - 38 edges
5. `FTSSidecar` - 27 edges
6. `IndexStats` - 23 edges
7. `CodeSight` - 23 edges
8. `chunk_file()` - 21 edges
9. `RepoStatus` - 20 edges
10. `Answer` - 20 edges

## Surprising Connections (you probably didn't know these)
- `StaticEmbedder` --uses--> `CodeSight`  [INFERRED]
  tests/test_holus_lineage.py → src/codesight/api.py
- `TestHolusLineageImport` --uses--> `CodeSight`  [INFERRED]
  tests/test_holus_lineage.py → src/codesight/api.py
- `TestHolusLineageSearchAndAttribution` --uses--> `CodeSight`  [INFERRED]
  tests/test_holus_lineage.py → src/codesight/api.py
...
Cohesion: 0.08
Nodes (19): APIEmbedder, Embedder, LocalEmbedder, _normalize_rows(), ndarray, Protocol, OpenAI embedding API backend — best quality, requires API key., Embed texts via OpenAI API in batches of 512. (+11 more)

### Community 2 - "EvalQuery"
Cohesion: 0.09
Nodes (28): _count_tokens(), EvalQuery, EvalResult, Eval harness for CodeSight search quality and token efficiency. Metrics:…, Estimate token count. Uses tiktoken if available, else len//4., A single eval query with its expected answer location., Aggregate metrics from running the eval harness., Run eval harness over a list of queries against an indexed store. Args:… (+20 more)
...
Cohesion: 0.10
Nodes (23): Configuration for the CodeSight search engine., Return expected embedding dimension for a model. Falls back to 384., resolve_embedding_dim(), get_embedder(), Embedding model wrapper — local (sentence-transformers) or API (OpenAI).…, Return a cached Embedder singleton. Args: model_name: Model identifier from the…, _get_reranker(), _get_voyage_client() (+15 more)
...
Cohesion: 0.05
Nodes (40): Architecture -- CodeSight, Chunking Strategy, Code Files — AST-Based (tree-sitter), Context Headers, Context Injection Integration (Added 2026-04-04), Data Flow: What's Local vs External, Deployment Tiers, Document Processing Pipeline (+32 more)
...
Cohesion: 0.18
Nodes (9): field_validator, Path, BaseModel, Runtime configuration., Clamp CNFB alpha to [0.0, 2.0]. Logs a warning if clamping occurred., ServerConfig, Verify the engine never writes to the indexed folder., After indexing, the source folder should have no new files. (+1 more)
...
### Community 18 - "SearchResult"
Cohesion: 0.14
Nodes (30): Exception, Public Python API for CodeSight. This is the single entry point for Streamlit,…, HolusImportStats, BaseModel, Result of one idempotent local import of a Holus export snapshot., CodeSight — AI-powered document search engine. Hybrid BM25 + vector retrieval…, Answer, ChunkRecord (+22 more)
...
Cohesion: 0.14
Nodes (14): Acceptance Criteria, Alternative A: Language Server Protocol (LSP), Alternative B: Improved regex patterns, Alternative C: Concrete Syntax Tree (CST) via tree-sitter, Alternatives Considered, AST-Based Chunking Strategy, Edge Cases & Failure Modes, Goals (+6 more)
...
### Community 24 - "test_holus_lineage.py"
Cohesion: 0.15
Nodes (13): MonkeyPatch, parametrize, engine(), _holus_export(), fixture, ndarray, Path, Contract tests for read-only Holus lineage ingestion. (+5 more)
...
Cohesion: 0.32
Nodes (16): HolusLineageRecord, parse_holus_lineage_export(), Any, Read-only adapter for Holus's versioned lineage export. The adapter consumes an…, Validate a Holus v1 export fully before returning data for storage., Validated, safe data needed to index one producer lineage node., _reject_unknown_fields(), _require_fields() (+8 more)
...
### Community 93 - ".set_meta"
Cohesion: 0.29
Nodes (3): setter, Persist source-specific provenance without changing the chunk schema., Return previously stored safe provenance for a lineage chunk.
...
### Community 111 - "TestPathTraversal"
Cohesion: 0.25
...
Cohesion: 0.23
Nodes (17): FastAPI, Request, api_key(), create_app(), documents_dir(), _env_bool(), _extract_key(), get_engine() (+9 more)
...
### Community 129 - "_detect_language"
Cohesion: 0.36
...
### Community 135 - "test_placeholder.py"
Cohesion: 0.33
Nodes (5): Public package export contract tests., The package root exposes a stable public import surface., Root exports point at the implementation classes callers should import., test_package_declares_expected_public_exports(), test_package_exports_resolve_to_canonical_objects()

### Community 136 - "CodeSight"
Cohesion: 0.08
Nodes (15): CodeSight, Any, callable, Hybrid BM25 + vector search. Auto-indexes if needed., Ask a question — search + LLM answer synthesis. Retrieves the top matching…, Check index status for this folder., Auto-index if not indexed, auto-refresh if stale, rebuild on model mismatch., Check if the configured embedding model differs from the indexed one. (+7 more)

### Community 137 - "Implementation Notes"
Cohesion: 0.33
Nodes (6): Code Sketch, Dependencies, Implementation Notes, Implementation Steps, Key Parameters, Supported Languages

## Knowledge Gaps
- **786 isolated node(s):** `codesight`, `$schema`, `framework`, `installCommand`, `buildCommand` (+781 more)
...
- **Why does `ChunkStore` connect `ChunkStore` to `FTSSidecar`, `EvalQuery`, `store.py`, `search.py`, `CodeSight`, `._delete_vectors_by_ids`, `.upsert_chunks`, `TestFTSQuerySanitization`, `ServerConfig`, `TestPathTraversal`, `.get_chunk_metadata`, `SearchResult`, `indexer.py`, `.set_meta`?**
  _High betweenness centrality (0.032) - this node is a cross-community bridge._
- **Why does `CodeSight` connect `CodeSight` to `test_mcp.py`, `get_backend`, `search.py`, `TestMCPSearchTool`, `ServerConfig`, `TestFTSQuerySanitization`, `ChunkStore`, `TestPathTraversal`, `SearchResult`, `server.py`, `test_holus_lineage.py`, `__main__.py`?**
  _High betweenness centrality (0.026) - this node is a cross-community bridge._
- **Why does `ServerConfig` connect `ServerConfig` to `EvalQuery`, `test_mcp.py`, `search.py`, `CodeSight`, `TestMCPSearchTool`, `TestFTSQuerySanitization`, `ChunkStore`, `TestPathTraversal`, `SearchResult`, `server.py`, `test_holus_lineage.py`, `indexer.py`?**
  _High betweenness centrality (0.022) - this node is a cross-community bridge._
- **Are the 28 inferred relationships involving `CodeSight` (e.g. with `ServerConfig` and `HolusImportStats`) actually correct?**
  _`CodeSight` has 28 INFERRED edges - model-reasoned connections that need verification._
...
- **Are the 25 inferred relationships involving `ServerConfig` (e.g. with `CodeSight` and `AskRequest`) actually correct?**
  _`ServerConfig` has 25 INFERRED edges - model-reasoned connections that need verification._
- **Are the 21 inferred relationships involving `SearchResult` (e.g. with `CodeSight` and `AskRequest`) actually correct?**
  _`SearchResult` has 21 INFERRED edges - model-reasoned connections that need verification._