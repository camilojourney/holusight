...
## Corpus Check
- 124 files · ~79,926 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1696 nodes · 2177 edges · 137 communities (125 shown, 12 thin omitted)
- Extraction: 95% EXTRACTED · 5% INFERRED · 0% AMBIGUOUS · INFERRED: 113 edges (avg confidence: 0.51)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `a0c27823`
...
Cohesion: 0.09
Nodes (14): field_validator, Path, BaseModel, Runtime configuration., Clamp CNFB alpha to [0.0, 2.0]. Logs a warning if clamping occurred., ServerConfig, indexed_engine(), fixture (+6 more)
...
Cohesion: 0.15
Nodes (27): Exception, Public Python API for CodeSight. This is the single entry point for Streamlit,…, Ask a question — search + LLM answer synthesis. Retrieves the top matching…, Hybrid BM25 + vector search. Auto-indexes if needed., CodeSight — AI-powered document search engine. Hybrid BM25 + vector retrieval…, Answer, ChunkRecord, IndexStats (+19 more)

### Community 19 - "_cnfb_boost"
Cohesion: 0.06
Nodes (30): _cnfb_boost(), Apply Query-Aware Multiplicative Filename Boost (CNFB, SPEC-009). Precomputes…, Reciprocal Rank Fusion across multiple ranked ID lists. Returns [(chunk_id,…, rrf_merge(), _make_cnfb_result(), _make_result(), Tests for the search module (RRF merging + reranker routing)., _rerank() calls local cross-encoder when backend='local'. (+22 more)
...
Cohesion: 0.16
Nodes (10): CodeSight, callable, Check index status for this folder., Auto-index if not indexed, auto-refresh if stale, rebuild on model mismatch., Check if the configured embedding model differs from the indexed one., Check if the index is older than the staleness threshold., AI-powered document search engine. Usage: engine =…, Index all documents in the folder. Handles code files, text files, PDFs, DOCX,… (+2 more)
...
### Community 35 - "vprf_enhance_query"
Cohesion: 0.17
...
Cohesion: 0.12
Nodes (12): doc_folder(), engine(), isolated_data_dir(), fixture, Tests for the CodeSight API surface that replaces the former MCP tools. The…, CodeSight.status() ↔ former MCP status tool., CodeSight.ask() — API extension beyond the original three MCP tools., Minimal document folder with one searchable Python file. (+4 more)
...
### Community 59 - ".delete_file_chunks"
Cohesion: 0.20
Nodes (5): Delete all chunks belonging to a file. Returns count deleted., Return {chunk_id: content_hash} for all chunks of a file., Validate a chunk_id against the allowlist regex. Returns the chunk_id if valid,…, Delete vectors from LanceDB using validated chunk IDs. Each chunk_id is…, Remove all chunks for a file from both stores.
...
### Community 70 - "repo_data_dir"
Cohesion: 0.47
Nodes (5): Path, Return the data directory for a given folder, creating parent dirs if needed.…, Return the SQLite FTS5 sidecar DB path for a given folder., repo_data_dir(), repo_fts_db_path()

### Community 71 - ".upsert_code_chunks"
Cohesion: 0.25
Nodes (4): Open or create the code_chunks LanceDB table with the Voyage vector dimension., Upsert code chunks into the code_chunks vector table and shared metadata store., Upsert shared chunk metadata into the FTS sidecar., Insert or replace a chunk in the metadata store.
...
Cohesion: 0.18
Nodes (12): _load_vercel_config(), Deployment regression tests for holusight.com static site., Vercel must serve a static entry point or holusight.com returns 404., Key marketing/docs pages must exist for holusight.com navigation., Keep Vercel deploy as a static landing page, not a Streamlit/FastAPI build., Marketing copy must not claim planned or unverified capabilities., Docker Compose's normalized model must protect the customer boundary., test_compose_resolves_customer_mount_and_production_environment() (+4 more)
...
### Community 109 - "Implementation Notes"
Cohesion: 0.40
Nodes (5): Dependencies, Implementation Notes, Implementation Steps, Key Parameters, Model Comparison
...
### Community 124 - "require_auth"
Cohesion: 0.20
Nodes (15): FastAPI, Request, api_key(), create_app(), documents_dir(), _env_bool(), _extract_key(), get_engine() (+7 more)
...
### Community 127 - "TestChunkIdSanitization"
Cohesion: 0.20
Nodes (7): Security tests for read-only invariant and input sanitization., Verify path traversal attacks are prevented., CodeSight rejects non-directory paths., CodeSight resolves symlinks and validates the real path., Verify that chunk IDs with injection attempts are rejected., TestChunkIdSanitization, TestPathTraversal
...
### Community 132 - "test_placeholder.py"
Cohesion: 0.33
...
- **Why does `ServerConfig` connect `ServerConfig` to `EvalQuery`, `test_mcp.py`, `TestMCPIndexTool`, `search.py`, `TestMCPSearchTool`, `TestFTSQuerySanitization`, `indexer.py`, `SearchResult`, `CodeSight`, `require_auth`, `TestChunkIdSanitization`?**
  _High betweenness centrality (0.024) - this node is a cross-community bridge._
- **Why does `ChunkStore` connect `ChunkStore` to `FTSSidecar`, `EvalQuery`, `search.py`, `.upsert_code_chunks`, `repo_data_dir`, `.upsert_chunks`, `TestFTSQuerySanitization`, `ServerConfig`, `.get_chunk_metadata`, `SearchResult`, `CodeSight`, `.delete_file_chunks`, `indexer.py`, `.set_meta`, `TestChunkIdSanitization`?**
  _High betweenness centrality (0.023) - this node is a cross-community bridge._
- **Why does `SearchResult` connect `SearchResult` to `EvalQuery`, `vprf_enhance_query`, `test_mcp.py`, `TestMCPIndexTool`, `search.py`, `TestMCPSearchTool`, `_cnfb_boost`, `CodeSight`, `TestMetadataBoost`?**
