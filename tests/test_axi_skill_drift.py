...
`.claude/skills/holus/SKILL.md` is generated from `src/holusight/axi_schema.py`
by `src/holusight/axi_skill_gen.py`. This test regenerates the skill text
...
`python -m holusight.axi_skill_gen` sees this test fail in the normal
...
from holusight.axi_skill_gen import SKILL_PATH, render_distribution_skill, render_skill


def test_skill_file_exists():
    assert SKILL_PATH.exists(), (
        f"{SKILL_PATH} is missing - run `python -m holusight.axi_skill_gen`"
    )
...
def test_committed_skill_matches_generated_skill():
    ...
    assert committed == generated, (
        ...
        "`src/holusight/axi_schema.py`. Run `python -m holusight.axi_skill_gen` "
